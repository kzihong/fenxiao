		...
		try {
			if (findAdmin != null) {
				...
				if (findAdmin.getStatus().intValue() == 0) {
					...
					/*
					 * String domain = this.request.getServerName(); String
					 * serverip = InetAddress.getLocalHost().getHostAddress();
					 * String systemName = "汉三信息微商城卡密系统"; String systemVersion =
					 * "V1.0.1"; String url =
					 * "http://systemapi.hansan.com/RecordServlet?"; url = url +
					 * "domain=" + domain + "&"; url = url + "serverip=" +
					 * serverip + "&"; url = url + "systemName=" + systemName +
					 * "&"; url = url + "systemVersion=" + systemVersion + "&";
					 * url = url + "clientip=" + ip; Connect.httpConnect(url);
					 */
				}
			} else {

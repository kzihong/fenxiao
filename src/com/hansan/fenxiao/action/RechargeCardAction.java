...
public class RechargeCardAction extends BaseAction {
	...
	public void list() {
		...
		if (StringUtils.isNotEmpty(status)) {
			...
			countHql = countHql + " and status=" + 0;
		}
		...
		List rechargeCardList = this.rechargeCardService.list(hql, this.page.getStart(), this.page.getPageSize(),
				new Object[0]);
		double moneycount = this.rechargeCardService.getCount();
		Map root = new HashMap();
		root.put("rechargeCardList", rechargeCardList);
		root.put("page", this.page);
		root.put("no", no);
		root.put("moneycount", moneycount);
	...
	public void ChargeForUser() {
		...
		try {
			if (cardIdStr == null|| userIdStr == null) {
				callbackData = BjuiJson.json("300", "参数错误", "", "", "", "", "", "");
			} else {
				int userId = Integer.parseInt(userIdStr);
				User findUser = (User) this.userService.findById(User.class,userId);
				
				RechargeCard findRechargeCard = (RechargeCard) this.rechargeCardService.getByNo(cardIdStr);
				if (findRechargeCard == null) {
					callbackData = BjuiJson.json("300", "充值卡不存在", "", "", "", "", "", "");
				}else {
					if(findRechargeCard.getStatus().intValue() == 1){
						callbackData = BjuiJson.json("300", "充值卡已经被使用", "", "", "", "", "", "");
					}else{
						...
						findRechargeCard.setUseUserNo(findUser.getPhone());
						this.rechargeCardService.saveOrUpdate(findRechargeCard);
						callbackData = BjuiJson.json("200", "充值成功", "", "", "", "true", "", "");
					}
				}
			}
		} catch (JSONException e) {

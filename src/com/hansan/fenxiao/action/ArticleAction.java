...
public class ArticleAction extends BaseAction {
	...
	public void promotePage() {
		request.setAttribute("article", articleService.findById(Article.class, 3));
		try {
			request.getRequestDispatcher("../user/promote.jsp").forward(request, response);
	...
	public void contact() {
		request.setAttribute("config", this.configService.findById(Config.class, 1));
		request.setAttribute("article", articleService.findById(Article.class, 4));
		try {
	...
	public void rechangePage() {
		request.setAttribute("article", articleService.findById(Article.class, 2));
		try {
			request.getRequestDispatcher("../user/rechargeCard.jsp").forward(request, response);
		} catch (ServletException | IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void detail() {
		...
		request.setAttribute("article", articleService.findById(Article.class, id));
		try {
			request.getRequestDispatcher("../user/articleDetail.jsp").forward(request, response);
		} catch (ServletException | IOException e) {
			e.printStackTrace();
		}
	}
	
	public void articleListPage() {
		List<Article> articles = this.articleService.getTopArticles();
		articles.addAll(this.articleService.getUnTopArticles());
		request.setAttribute("list", articles);
		try {
			request.getRequestDispatcher("../user/articleList.jsp").forward(request, response);
		} catch (ServletException | IOException e) {
			e.printStackTrace();
		}
	}
	...
	public void getTopArticles() {
		String callbackData = "";
		List<Article> articles = this.articleService.getTopArticles();
		callbackData = JSONArray.toJSONString(articles);
		PrintWriter out = null;
		try {
			out = this.response.getWriter();
		} catch (IOException e1) {
 ...
 * Qualified Name: com.hansan.fenxiao.action.ArticleAction JD-Core Version:
 * 0.6.0
 */
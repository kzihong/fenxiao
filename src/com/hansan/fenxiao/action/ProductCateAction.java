...
public class ProductCateAction extends BaseAction {
	...
	public void getProductcate() {
		...
		String callbackData = JSONArray.toJSONString(list);
	...
	public void update() {
		...
		try {
			if (this.productCate == null) {
				...
				findProductCate.setFirstLevel(this.productCate.getFirstLevel());
				findProductCate.setSecoundLevel(this.productCate.getSecoundLevel());
				findProductCate.setThirdLevel(this.productCate.getThirdLevel());
				findProductCate.setInfo(this.productCate.getInfo());
	...
	public void betting() {
		List<ProductCate> plist = this.productCateService.list("from ProductCate where deleted=0 and fatherId = 0 ");
		String html = "";
		for (ProductCate productCate : plist) {
			html += "<div class='panel panel-default'><div class='panel-heading'><a data-toggle='collapse' data-parent='#accordion'href='#";
			html += "collapse"+productCate.getId()+"'><h4 class='panel-title'>";
			html += productCate.getName();
			html += "</h4></a></div><div id='collapse"+productCate.getId()+"' class='panel-collapse collapse'><div class='panel-body'><ul class='list-group'>";
			List<ProductCate> productCates = this.productCateService
					.list("from ProductCate where deleted=0 and fatherId = " + productCate.getId());
			for (ProductCate productCate2 : productCates) {
				html += "<a href='productList?c=";
				html +=productCate2.getId();
				html +="'><li class='list-group-item'>";
				html += productCate2.getName();
				html += "<span class='badge'>";
				html += productCate2.getInfo();
				html += "</span></li></a>";
			}
			html += "</ul></div></div></div>";
		}
		request.setAttribute("html", html);
		request.setAttribute("xz", articleService.findById(Article.class, 1));
		request.setAttribute("yx", articleService.findById(Article.class, 5));
		
		try {
			request.getRequestDispatcher("betting.jsp").forward(request, response);

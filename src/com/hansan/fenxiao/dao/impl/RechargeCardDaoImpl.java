...
public class RechargeCardDaoImpl extends BaseDaoImpl<RechargeCard> implements IRechargeCardDao {
	...
	public double getCount() {
		String sql = "SELECT bean FROM RechargeCard bean WHERE DATE_FORMAT(bean.useTime,'%Y%m')=DATE_FORMAT(CURDATE(),'%Y%m')";
		List<RechargeCard> rechargeCards = getSession().createQuery(sql).list();
		double d= 0;
		for(RechargeCard rechargeCard : rechargeCards){
			d += rechargeCard.getMoney();
		}
		return d;
	}
}